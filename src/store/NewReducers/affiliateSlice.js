...
export const fetchReferredList = createAsyncThunk("affiliate/fetchReferredList", async ({idToken, affiliateId, activeTab}, {dispatch, rejectWithValue}) => {
  try {
...
export const fetchPushleadsChartData = createAsyncThunk("affiliate/fetchPushleadsChartData", async ({idToken, user_id}, {dispatch, rejectWithValue}) => {
  try {
...
export const fetchExportHistory = createAsyncThunk("affiliate/fetchExportHistory", async ({idToken, url}, {dispatch, rejectWithValue}) => {
  try {
    const response = await getExportHistory(idToken, url);
    if (response.status < 399) {
      return response.data;
    } else {
