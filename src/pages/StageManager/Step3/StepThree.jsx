...
const StepThree = () => {
  ...
  const stagesOption = location.pathname === "/support/step-3" ? ["Stage 1 Pass", "Stage 2 Pass", "Stage 3 Pass"] : [];
  ...
  async function fetchStageList(idToken, pageNo, pageSize, searchText, status, dates) {
    ...
    if (location.pathname === "/support/step-3") {
      category = "STEP 3";
      type = stage === "Stage 1 Pass" ? "Stage 1 Pass" : stage === "Stage 2 Pass" ? "Stage 2 Pass" : stage === "Stage 3 Pass" ? "Stage 3 Pass" : "stage 1 Pass";
      query += `&type=${type}&category=${category}`;
    } else {
      url = location.pathname === "/support/funded" ? "v2/get/funded/list/" : "v2/get-payout/";
    }
...
function ExpandedRowData({record}) {
  ...
  useEffect(() => {
    if (record.id === isExpandable) {
      let flag = location.pathname === "/support/step-1" || location.pathname === "/support/step-2" ? true : false;
